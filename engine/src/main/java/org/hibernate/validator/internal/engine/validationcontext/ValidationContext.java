...
public interface ValidationContext<T> {
	...
	/**
	 * Creates a constraint violation {@link ConstraintViolation} based on passed parameters and adds it
	 * to a set of failed constrains.
	 */
	...
	/**
	 * @return a set of constraint vilations created with {@link ValidationContext#addConstraintFailure(ValueContext, ConstraintViolationCreationContext, ConstraintDescriptor)}.
	 */
	Set<ConstraintViolation<T>> getFailingConstraints();

	/**
	 * @return created constraint validator context based on passed constraint descriptor and path.
	 */
	AbstractConstraintValidatorContext createConstraintValidatorContextFor(ConstraintDescriptorImpl<?> constraintDescriptor, PathImpl path);
}
